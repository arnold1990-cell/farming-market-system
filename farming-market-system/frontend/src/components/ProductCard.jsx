...
export default function ProductCard({ product, onAdd }) {
  ...
  return (
    <article className="group overflow-hidden rounded-[28px] border border-white/80 bg-white px-2.5 pb-3 pt-2.5 shadow-[0_16px_36px_rgba(15,23,42,0.08)] transition duration-200 active:scale-[0.99]">
      <div className="relative rounded-[22px] bg-[#EFF8F1] p-2.5">
        <img
          src={imgSrc}
        ...
        />
        {discount > 0 ? <span className="absolute left-4 top-4 rounded-full bg-red-500 px-2 py-1 text-[10px] font-semibold text-white">-{discount}%</span> : null}
        <button type="button" className="absolute right-4 top-4 rounded-full bg-white/95 p-1.5 text-gray-600 shadow-sm">
          <Heart size={14} />
        </button>
      </div>
      ...
      <div className="space-y-2 px-1 pt-3">
        ...
        <p className="inline-flex items-center gap-1 text-[11px] text-gray-500"><MapPin size={11} />{product.location}</p>
        {harvestDate ? <p className="text-[11px] font-medium text-emerald-700">{harvestLabel} · {harvestDate}</p> : <p className="text-[11px] font-medium text-emerald-700">{harvestLabel}</p>}
        <div className="flex items-center justify-between">
          ...
          <span className={`rounded-full px-2 py-1 text-[10px] font-semibold ${stockLevel <= 5 ? 'bg-amber-100 text-amber-700' : 'bg-emerald-100 text-emerald-700'}`}>
