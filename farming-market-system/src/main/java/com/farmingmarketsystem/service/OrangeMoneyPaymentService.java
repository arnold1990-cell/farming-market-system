...
public class OrangeMoneyPaymentService {
    ...
    public PaymentDtos.Response initiatePayment(Long orderId, String customerPhone) {
        ...
        payment.setProvider(PaymentProvider.ORANGE_MONEY);
        payment.setStatus(PaymentStatus.INITIATED);
        payment.setAmount(order.getTotalAmount());
        payment.setCustomerPhone(customerPhone);
        payment.setTransactionReference("OM-" + UUID.randomUUID().toString().substring(0, 8).toUpperCase());
    ...
    public boolean isEnabled() {
        return properties.getOrangeMoney().isEnabled();
    }
}
